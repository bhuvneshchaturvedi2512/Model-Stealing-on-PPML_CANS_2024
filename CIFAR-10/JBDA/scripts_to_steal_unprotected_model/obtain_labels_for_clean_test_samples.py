...
from trainer_surrogate import get_train_set_cifar10
...
NUM_SAMPLES = 1000 #No. of clean samples already available to the adversary

def getLabel(expected_quantized_prediction):
...
test_set = get_train_set_cifar10(dataset="CIFAR10", datadir=CURRENT_DIR.joinpath(".datasets/"))
...
for image_index in range(NUM_SAMPLES):
    with open('Y_original.txt','a') as fp:
        fp.write(str(y_numpy[image_index]) + '\n')
...
for image_index in range(NUM_SAMPLES):
    ...
    for i in range(len(test_x_numpy[0])):
        for j in range(len(test_x_numpy[0][0])):
            for k in range(len(test_x_numpy[0][0][0])):
                with open('X_clean_test_samples.txt','a') as fp:
    ...
    predicted_label = getLabel(expected_quantized_prediction[0])
    
    with open('Y_clean_test_samples.txt','a') as fp:
        fp.write(str(predicted_label) + '\n')
        
print("Predictions obtained for clean test samples")
