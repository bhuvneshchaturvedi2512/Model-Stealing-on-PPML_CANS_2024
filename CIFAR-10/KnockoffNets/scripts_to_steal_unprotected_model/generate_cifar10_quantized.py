...
from cifar10_test_quantized import get_test_set_cifar10
...
test_set = get_test_set_cifar10(dataset="CIFAR10", datadir=CURRENT_DIR.joinpath(".datasets/"))
...
)(torch_model, x[0:100], configuration=configuration, rounding_threshold_bits=6, p_error=0.01)
...
for image_index in range(NUM_SAMPLES):
    ...
    for i in range(len(q_x_numpy[0])):
        for j in range(len(q_x_numpy[0][0])):
            for k in range(len(q_x_numpy[0][0][0])):
                with open('X_cifar10_test.txt','a') as fp:
    ...
    with open('Y_cifar10_test.txt','a') as fp:
