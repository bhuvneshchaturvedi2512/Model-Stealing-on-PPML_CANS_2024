...
with open('X_stl10_test.txt','r') as fp:
	lines_x = fp.readlines()
	
with open('Y_stl10_test.txt','r') as fp:
	lines_y = fp.readlines()
		...
		if(channel != ''):
			...
			for row in temp_row:
				if(row != ''):
					...
					list_x_row.append(np.array(list_x_column, dtype=np.float32))
			list_x_channel.append(list(list_x_row))
	x_stl10.append(np.array(list_x_channel, dtype=np.float32))

for line_y in lines_y:
	temp = line_y.strip()
	if(temp != ''):
		y_stl10.append(int(temp))

y_stl10 = to_categorical(y_stl10, 10)

x_stl10_new = []
for i in range(len(lines_y)):
	x_stl10_new.append(np.transpose(x_stl10[i],(1, 2, 0)).tolist())

x_cifar10_new = np.array(x_cifar10_new)
y_cifar10 = np.array(y_cifar10)

x_stl10_new = np.array(x_stl10_new)
y_stl10 = np.array(y_stl10)
...
with tf.device('/device:GPU:0'): # /device:GPU:0 for training in GPU
	model.fit(x_stl10_new, y_stl10, epochs=100, batch_size=64, verbose=1, validation_data=(x_cifar10_new, y_cifar10), callbacks=callbacks)

#model_save_path = "/home/seal-lab-workstation/space_submission_2023/Model_Stealing_in_FHE/concrete_cifar_brevitas_training/KnockoffNets/stl10_with_quantized_datapoints/stolen_model.h5"
