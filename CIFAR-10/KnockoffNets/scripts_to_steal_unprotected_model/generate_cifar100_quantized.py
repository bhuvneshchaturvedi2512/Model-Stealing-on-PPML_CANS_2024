...
from cifar10_test_quantized import get_train_set_cifar10, get_train_set_cifar100
...
# Import and load the CIFAR test dataset
train_set_cifar100 = get_train_set_cifar100(dataset="CIFAR100", datadir=CURRENT_DIR.joinpath(".datasets/"))
train_loader_cifar100 = DataLoader(train_set_cifar100, batch_size=50000, shuffle=False)

# Get the first sample
samples_data = []
count = 0

for i, data in enumerate(train_loader_cifar100):
    samples_data.append(data[0])
    samples_data.append(data[1])
    count = count + 1
    if(count == 1):
        break
x_cifar100, y_cifar100 = samples_data

NUM_SAMPLES = len(x_cifar100)
...
for image_index in range(NUM_SAMPLES):
    ...
    for i in range(len(q_x_numpy[0])):
        for j in range(len(q_x_numpy[0][0])):
            for k in range(len(q_x_numpy[0][0][0])):
                with open('X_cifar100_test.txt','a') as fp:
    ...
    with open('Y_cifar100_test.txt','a') as fp:
