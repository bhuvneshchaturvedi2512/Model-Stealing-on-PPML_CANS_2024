...
from cifar10_test_quantized import get_train_set_cifar10, get_test_set_cifar10
...
# Import and load the CIFAR test dataset
test_set_cifar10 = get_test_set_cifar10(dataset="CIFAR10", datadir=CURRENT_DIR.joinpath(".datasets/"))
test_loader_cifar10 = DataLoader(test_set_cifar10, batch_size=10000, shuffle=False)

# Get the first sample
samples_data = []
count = 0

for i, data in enumerate(test_loader_cifar10):
    samples_data.append(data[0])
    samples_data.append(data[1])
    count = count + 1
    if(count == 1):
        break
x_cifar10, y_cifar10 = samples_data

NUM_SAMPLES = len(x_cifar10)
...
for image_index in range(NUM_SAMPLES):
    ...
    with open('cifar10_max_sp.csv','a') as fp:
