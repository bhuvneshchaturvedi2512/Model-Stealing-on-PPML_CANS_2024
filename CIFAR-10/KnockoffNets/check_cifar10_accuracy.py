...
for line_x in lines_x:
	...
	x_test.append(np.array(list_x_channel, dtype=np.float32))
...
)(torch_model, x_train, configuration=configuration, rounding_threshold_bits=6, p_error=0.01)
...
for image_index in range(NUM_SAMPLES):
    test_x_numpy = x_test[image_index : image_index + 1]
    ...
    y_obtained.append(predicted_label)
    
    with open('Y_cifar10_test_obtained.txt','a') as fp:
        fp.write(str(predicted_label) + '\n')  
        
    if((image_index + 1) % 1000 == 0):
        print("First " + str(image_index + 1) + " samples processed")
...
for image_index in range(NUM_SAMPLES):
    if(y_obtained[image_index] == y_test[image_index]):
        count = count + 1

accuracy = count/NUM_SAMPLES
        
print(f"Accuracy on benign samples: {np.around(accuracy*100, 2)}")
