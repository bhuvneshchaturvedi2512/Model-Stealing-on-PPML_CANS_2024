...
def manage_dataset_emnist(train_kwargs, test_kwargs):
    """Get training and test parts of MNIST data-set."""

    transform = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize((0.1733,), (0.3255,)),
            transforms.Lambda(torch.flatten),
        ]
    )
    ...
    # Manage data-sets
    dataset1 = datasets.EMNIST(root=root_path, split = 'digits', download=True, train=True, transform=transform)
    dataset2 = datasets.EMNIST(root=root_path, split = 'digits', download=True, train=False, transform=transform)
    train_loader = torch.utils.data.DataLoader(dataset1, **train_kwargs)
    test_loader = torch.utils.data.DataLoader(dataset2, **test_kwargs)
...
def compile_and_test(
    ...
    for image_index in range(test_data_length):
        ...
        for i in range(len(reduced_test_data[image_index])):
            ...
            with open('X_emnist_test.txt','a') as fp:
        ...
        with open('X_emnist_test.txt','a') as fp:
            fp.write("\n")
        with open('Y_emnist_test.txt','a') as fp:
...
train_loader_emnist, test_loader_emnist = manage_dataset_emnist(train_kwargs, test_kwargs)
...
"""for image_index in range(len(test_data)):
    count = 0
    for i in range(len(test_data[image_index])):
        count = count + 1
        with open('X_mnist_test.txt','a') as fp:
            fp.write(str(test_data[image_index][i]))
            if(count != 28):
                fp.write(",")
            else:
                fp.write("#")
    ...
    with open('X_mnist_test.txt','a') as fp:
        fp.write("\n")
    with open('Y_mnist_test.txt','a') as fp:
        fp.write(str(int(test_target[image_index])) + '\n')
    if(image_index % 100 == 0):
        print("First " + str(image_index) + " samples written")"""
    #exit(0)
   
# Prepare FashionMNIST tests
test_data = np.zeros((len(train_loader_emnist.dataset), img_size * img_size))
test_target = np.zeros((len(train_loader_emnist.dataset), 1))
idx = 0

for data, target in tqdm(train_loader_emnist):
    target_np = target.cpu().numpy()
    for idx_batch, im in enumerate(data.numpy()):
        test_data[idx] = im
        test_target[idx] = target_np[idx_batch]
