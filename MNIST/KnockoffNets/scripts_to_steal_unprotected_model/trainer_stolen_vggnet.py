...
with open('X_fmnist_test.txt','r') as fp:
	lines_x = fp.readlines()
	
with open('Y_fmnist_test.txt','r') as fp:
	lines_y = fp.readlines()
...
for line_x in lines_x:
	...
	for row in temp_row:
		if(row != ''):
			...
			list_x_row.append(np.array(list_x_column, dtype=np.float32))
	list_x_channel.append(list(list_x_row))
	x_fmnist.append(np.array(list_x_channel, dtype=np.float32))

for line_y in lines_y:
	temp = line_y.strip()
	if(temp != ''):
		y_fmnist.append(int(temp))

y_fmnist = to_categorical(y_fmnist, 10)


x_fmnist_new = []
for i in range(len(lines_y)):
	x_fmnist_new.append(np.transpose(x_fmnist[i],(1, 2, 0)).tolist())


x_mnist = tf.expand_dims(x_mnist_new, axis=-1)
x_fmnist = tf.expand_dims(x_fmnist_new, axis=-1)
...
with tf.device('/device:GPU:0'): # /device:GPU:0 for training in GPU
	model.fit(x_fmnist, y_fmnist, epochs=100, batch_size=64, verbose=1, validation_data=(x_mnist, y_mnist), callbacks=callbacks)
